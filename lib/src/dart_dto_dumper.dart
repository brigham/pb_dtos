  ...
  Map<String, dynamic> createContext(CollectionSchema collectionSchema) {
    ...
    return {
      ...
      'type_definitions': collection.fields
          ...
          .toList(),
      'fields': collection.fields.indexed.map((idxAndField) {
        var (idx, field) = idxAndField;
        var camelized = lowerCamelize(field.name);
        var enumName =
            {'name', 'index', 'hashCode', 'values'}.contains(camelized)
            ? '${camelized}_'
            : camelized;
        ...
        var dartPatchType = dartType.endsWith("?") ? dartType : "$dartType?";
        ...
        var relationClassName = field.data['type'] == 'relation'
            ? toClassName(schema.byId[field.data['collectionId']]!.model.name)
            : null;
        ...
        switch ((relationClassName != null, hasMulti)) {
          ...
          case (true, false):
            filterComparisonBuilder = '${relationClassName}ComparisonBuilder';
          case (false, true):
        ...
        return {
          ...
          'is_field_modifiable': const {'text', 'editor'}.contains(field.type),
        };
      }).toList(),
      ...
      'indexes': collectionSchema.indexes.map((index) {
        ...
        if (parameters.isNotEmpty) {
          expressions.add({
            'code':
                'f.${parameters[0]['name']}().equal(${parameters[0]['name']})',
          });
          for (var i = 1; i < parameters.length; i++) {
            final p = parameters[i];
            expressions.add({
              'code':
                  "if (${p['name']} != null) f.${p['name']}().equal(${p['name']})",
            });
          }
        }
